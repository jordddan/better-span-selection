...
class RobertaNegSampleForTokenClassificationCL(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states1 = self.get_hidden_states(input_ids,attention_mask,start_pos)
        hidden_states2 = self.get_hidden_states(input_ids,attention_mask,start_pos)

        sim = self.get_sim(hidden_states1,hidden_states2, labels_mat)

        logits1 = self.cls(hidden_states1)
        logits2 = self.cls(hidden_states2)
        loss_func = CrossEntropyLoss()
        loss = loss_func(logits1.view(-1, self.num_labels), labels_mat.view(-1)) if labels_mat is not None else None
    ...
    def get_sim(self, hs1, hs2, labels):

        import pdb
        pdb.set_trace()

        batch_size, n, _, hidden_size = hs1.shape
